...
pub(crate) struct ConfigData {
  ...
  pub(crate) ack_timeout_millis: u32,
...
pub struct Config {
  ...
  ack_timeout_millis: Option<u32>,
  ack_random_factor: Option<f32>,
...
/// TODO
#[derive(Debug, Clone, Copy)]
pub struct AckRandomFactor(f32);
impl AckRandomFactor {
  /// TODO
  pub fn try_new(factor: f32) -> Option<Self> {
    match factor {
      | n if n >= 1.0 => Some(Self(n)),
      | _ => None,
    }
  }
}
...
impl Config {
  ...
  ///
  /// The default value is 0, although it is best practice to set this to something else.
  /// This could be a random integer, or a machine identifier.
  ///
  /// _e.g. if you're developing a swarm of smart CoAP-enabled thermostats,
  /// each one would ideally have a distinct token_seed._
  ///
  /// The purpose of the seed is to make it more difficult for an observer of
  /// unencrypted CoAP traffic to guess what the next token will be.
  ///
  /// Tokens are generated by smooshing together the 2-byte seed
  /// with an 8-byte timestamp obtained from the system clock
  ///
  /// ```text
  /// Core.token_seed
  /// ||
  ...
  /// Set the transmission rate that we should do our best not to exceed when waiting for:
  ...
  /// Set the number of concurrent requests we are allowed to have
  /// in-flight for each server.
  ///
  ...
  /// Set the maximum number of times we should re-send confirmable
  /// requests before getting a response.
  ///
  /// The default value is 4 attempts
  pub fn max_tries(mut self, max_tries: crate::retry::Attempts) -> Self {
    self.max_retransmit_attempts = Some(max_tries.0);
    self
  }

  /// Set the factor used to vary our CON retry timings.
  ///
  /// See [`AckRandomFactor`] for more details
  ///
  /// The default value is 1.5
  pub fn ack_random_factor(mut self, factor: AckRandomFactor) -> Self {
    self.ack_random_factor = Some(factor.0);
    self
  }

  /// Set the maximum amount of time we should wait to respond to incoming
  /// multicast requests.
  ///
  ...
  /// Set the minimum amount of time we should wait before resending
  /// CON requests that haven't been ACKed.
  ///
  /// The default value is 2 seconds.
  pub fn ack_timeout(mut self, ack_timeout: Milliseconds<u32>) -> Self {
    self.ack_timeout_millis = Some(ack_timeout.0);
    self
  }
}
...
impl From<Config> for ConfigData {
  fn from(Config { token_seed,
                   ...
                   nstart,
                   probing_rate_bytes_per_sec, }: Config)
          -> Self {
    ConfigData { token_seed: token_seed.unwrap_or(0),
                 ack_timeout_millis: ack_timeout_millis.unwrap_or(2_000),
                 ack_random_factor: ack_random_factor.unwrap_or(1.5),

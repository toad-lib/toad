...
impl Socket for UdpSocket {
  ...
  fn connect<A: ToSocketAddrs>(&mut self, addr: A) -> nb::Result<(), Self::Error> {
    convert_socket_addrs(addr)
       .ok_or_else(|| nb::Error::Other(Error::new(ErrorKind::InvalidInput, "invalid socket addrs".to_string())))
...
fn convert_socket_addrs<A: ToSocketAddrs>(a: A) -> Option<Vec<std::net::SocketAddr>> {
  a.to_socket_addrs().ok().map(|iter| iter.map(|addr| match addr {
    SocketAddr::V4(sock) => {
    ...
    SocketAddr::V6(sock) => {
      ...
      let ip = std::net::Ipv6Addr::new(ip[0], ip[1], ip[2], ip[3], ip[4], ip[5], ip[6], ip[7]);
